...
export default function Snackbar({
  ...
  return (
    <div
      className={`${snackBarOpen && (type === "signed-in" || type === "signed-up") ? "" : "hidden"} fixed top-[90%] max-sm:top-[85%] z-40 flex flex-row max-sm:flex-col justify-between animate-in gap-2 align-middle items-center px-3 py-2 border rounded-xl border-gray-200 shadow-lg bg-white`}
    >
      <p className="max-sm:text-sm">Logged in as: {email}</p>
      {type === "signed-in" ? (
        <Link
          href="/orders"
