	...
	return (
		<div className="flex flex-row justify-between items-center border-b border-gray-300 pb-4">
			<div className="flex flex-row gap-4 h-full">
				<img
					className="object-cover w-24 h-24 rounded-xl aspect-square cursor-pointer"
					src={cartItem.image}
					onClick={() => {
				...
				<div className="flex flex-col h-full justify-between">
					<div className="flex flex-col">
						<h1 className="text-xl font-normal">{cartItem.name}</h1>
						<p className="text-gray-500">
							Price {"(1 unit)"}: Rs. {cartItem.price}
						</p>
					</div>
					<p>Total: Rs. {cartItem.price * cartItem.purchaseQuantity}</p>
				</div>
			</div>
